...
class SpeedRing(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        # ========== 60 FPS Interpolation Loop MIT CLEANUP ==========
        self._clock_event = Clock.schedule_interval(self._interpolate_values, 1/60.0)
    ...
    # -------------- Clean-Up funktionen gegen UI Frezzes ---------------------
    def on_parent(self, instance, parent):
        """Cleanup wenn Widget aus parent entfernt wird"""
        if parent is None:  # Widget wird entfernt
            self._cleanup()
    
    def _cleanup(self):
        """Stoppt den Clock-Event"""
        if hasattr(self, '_clock_event'):
            try:
                self._clock_event.cancel()
                print("SpeedRing: Clock-Event gestoppt")
            except:
                pass
    
    def __del__(self):
        """Fallback Cleanup beim Garbage Collection"""
        try:
            self._cleanup()
        except:
            pass