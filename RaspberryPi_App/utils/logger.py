...
class Datalogger: 
    ...
    def frame_logging(self, sensor):        # logged einen Frame in die DB
        ...
        gps_motion = False
        if sensor.gps_firstfix and sensor.fusion_speed is not None:    # GPS Bewegung erkennen
            gps_motion = sensor.fusion_speed > 1.5
        ...
        self.cursor.execute("""     
                INSERT INTO sensor_log (
                    pi_timestamp, drive_time, drive_distance, fusion_speed, raw_speed, fusion_alt,  
                    bmp_temp, bmp_pressure, g_lat, g_long, g_vert, lean_deg, heading_deg, pitch_deg,
                    gps_firstfix, gps_lat, gps_long, gps_alt, gps_heading,
                    batt_voltage, batt_temp1, batt_temp2, batt_temp3, batt_temp4,
        ...
        if self.publisher is not None:
            ...
            if not hasattr(self, '_last_ws_push'):
                self._last_ws_push = 0
            
            now_ms = datetime.datetime.now().timestamp() * 1000
            if now_ms - self._last_ws_push >= 100:   # 15 Hz Rate = UI Update Rate
                self._last_ws_push = now_ms
                
                self.publisher.publish({    # Objekt mit Methode "publish" --> aufrufen
                    "drive_time": drive_time,
                    # "drive_distance_m": round(self.drive_distance, 2),
                    # "altidute": sensor.fusion_alt,
                    # "temp": sensor.bmp_temp,
                    "speed": sensor.fusion_speed,
                    # "heading": sensor.gps_heading,
                    # "firstfix": int(sensor.gps_firstfix),
                    "lean_deg": sensor.lean_deg,
                    "pitch_deg": sensor.pitch_deg,
                    "batt_voltage": sensor.batt_voltage,
                    "max_batt_temp": max_temp
                    # auskommentierte Daten --> für zukünftigen Ausbau
                })
