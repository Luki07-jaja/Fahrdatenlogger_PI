...
class Sensordata:
    ...
    gps_lat: float = 0.0
    gps_alt: float = 0.0
    fusion_speed: float = 0.0
    gps_speed_raw: float = 0.0
...
START_BYTE = 0xAA
END_BYTE   = 0x55
FRAME_SIZE = 91     # passt zu: <I + 20f + ? + B> = 4 + 80 + 1 + 1 = 86 payload, + start/len/crc/end => 91
...
def unpack_frame(logger, data: bytes, last_esp_counter):
    structure = "<Ifffffffffffffff?fffffB"  # 32 bit int + 20 Floats + ? = 1xbool + 1 x 8 Bit int
    ...
    sensor.fusion_alt = round(sensor.fusion_alt, 2)
    ...
    sensor.fusion_speed = round(sensor.fusion_speed, 2)
    sensor.gps_speed_raw = round(sensor.gps_speed_raw, 3)
    ...
    print(f"Höhe: {sensor.fusion_alt:.2f} m")
    ...
    print(f"GPS: lat={sensor.gps_lat:.6f}, lon={sensor.gps_long:.6f}, speed={sensor.fusion_speed:.2f} km/h")
